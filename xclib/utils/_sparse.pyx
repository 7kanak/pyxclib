...
def rank_data(b):
    if b.size == 0:
        return _np.array([], dtype=np.int_)
    sorter = _np.argsort(b, kind='mergesort')
    inv = _np.empty(b.size, dtype=np.int_)
    inv[sorter] = _np.arange(sorter.size, dtype=np.int_)
    return inv+1
...
def _rank(data, indices, indptr):
    ...
    cdef Py_ssize_t idx
    cdef np.ndarray[np.int_t, ndim=1] rank = _np.empty(data.size, dtype=np.int_)
...
def _topk(data, indices, indptr, k, pad_ind, pad_val):
    ...
    cdef np.ndarray[np.int_t, ndim=2] ind = _np.full((num_rows, k), pad_ind, np.int_, 'C')
